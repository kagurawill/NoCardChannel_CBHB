...
public class RSAUtils {
    ...
    public static String encrypt(String paramstr, String publicKey) {
        try {
            byte[] cipherData = RSAUtils.encryptByPublicKey(paramstr.getBytes(ChanlConstant.CHARSET_GBK), publicKey);
    ...
    public static String decrypt(String cipherData, String privateKey) {
        try {
            ...
            String result = new String(cipher, ChanlConstant.CHARSET_GBK);
            System.out.println("解密结果：" + new String(result.getBytes(ChanlConstant.CHARSET_GBK), ChanlConstant.CHARSET_UTF8));
            return result;
        } catch (Exception e) {
            e.printStackTrace();
        }
